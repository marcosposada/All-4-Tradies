 ...
 */
class SolrBridge_Extend_Model_Solr_Blog extends SolrBridge_Solrsearch_Model_Solr
{
	/**
	...
	protected function preparePagingAndSorting()
	{
		...
		$toolbarBlock = Mage::app()->getLayout()->getBlockSingleton("sbsext/html_pager");
		
		//print_r($toolbarBlock);
		//die();
		...
		$page = Mage::helper('solrsearch')->getParam('p');
		if(!empty($page) && is_numeric($page)){
			$currentPage = $page;
		}
	
		//$limit = $toolbarBlock->getLimit();
		$limit = Mage::helper('solrsearch')->getParam('limit');
