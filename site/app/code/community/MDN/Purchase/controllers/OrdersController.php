...
class MDN_Purchase_OrdersController extends Mage_Adminhtml_Controller_Action {
    ...
    public function PrintAction() {
        ...
        try {
            ...
            $file = 'var/purchaseorders/'.$order->getpo_order_id().'.pdf';
            $this->_prepareDownloadResponse(mage::helper('purchase')->__('Purchase Order #') . $order->getpo_order_id() . '.pdf',  $file, 'application/pdf');
        } catch (Exception $ex) {
            Mage::getSingleton('adminhtml/session')->addError($this->__('An error occured') . ' : ' . $ex->getMessage());
            $this->_redirect('Purchase/Orders/Edit', array('po_num' => $order->getId()));
        }
    ...
    protected function _prepareDownloadResponse($fileName, $content, $contentType = 'application/octet-stream', $contentLength = null) {
        error_reporting(-1);
        Mage::log('Downloading');
        $this->getResponse()
                ->setHttpResponseCode(200)
                ->setHeader('Pragma', 'public', true)
                ->setHeader('Cache-Control', 'must-revalidate, post-check=0, pre-check=0', true)
                ->setHeader('Content-type', $contentType, true)
                ->setHeader('Content-Length', filesize($content))
                ->setHeader('Content-Disposition', 'attachment; filename=' . $fileName)
                ->setHeader('Content',$fileName)
                ->setBody(file_get_contents($content));
        Mage::log($content);
    }
