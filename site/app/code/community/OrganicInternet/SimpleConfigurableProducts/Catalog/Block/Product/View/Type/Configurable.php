    ...
    extends Mage_Catalog_Block_Product_View_Type_Configurable
{
    public function getJsonConfig()
    {
        ...
        foreach ($this->getAllowProducts() as $product) {
            ...
            $childProducts[$productId]["productCode"] = $this->helper('catalog/output')->productAttribute($product, nl2br($product->getErpSupplierSku()), 'erp_supplier_sku');
            $childProducts[$productId]["productSku"] = $this->helper('catalog/output')->productAttribute($product, nl2br($product->getSku()), 'sku');
        ...
        $config['productCode'] = $p->getErp_supplier_sku();
        $config['sku'] = $p->getSku();
