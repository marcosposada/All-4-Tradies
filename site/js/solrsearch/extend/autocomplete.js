...
Autocomplete.prototype = {
  ...
  fixPositionBk: function()
  {
	...
	this.container.setStyle({width:'100%', height: '100%', padding: '40px 0'});
	//$(this.box).setStyle({ top: (top) + 'px', left: (left) + 'px' });
	$(this.box).setStyle({ top: (0) + 'px', left: (0) + 'px', width: '100%', height: '100%'});
    //this.closebutton.setStyle({ top: '-10px', left: (boxWidth - 12)+ 'px' });
    this.closebutton.setStyle({ top: '3px', right: (0)+ 'px', zIndex: 999999 });
  },
  ...
  suggest: function() {
    ...
    if(this.suggestions.length > 0){
    	...
    	//CMS PAGE
    	if(this.suggestCmsPages.length > 0)
    	{
    		var viewAllCmsPageUrl = this.options.searchResultUrl+'/cms/?q='+this.currentKeyword;
    		var cmspageDivider = new Element('div').addClassName('suggest_category_items suggest_divider');
    		var cmspageDividerText = new Element('span').update('Pages');
    		cmspageDivider.appendChild(cmspageDividerText);
    		var viewAllCmsPageResult = new Element('span').addClassName('view-all-category').update('<a href="'+viewAllCmsPageUrl+'">'+'View all page result'+'</a>');
    		cmspageDivider.appendChild(viewAllCmsPageResult);
    		this.rightSideBar.appendChild(cmspageDivider);
    		
    		for(var key_cmspage in this.suggestCmsPages)
        	{
        		if(!isNaN(key_cmspage)){
        			var cmsPage = this.suggestCmsPages[key_cmspage];
        			
        			var cmsPageName = cmsPage.name_varchar;
        			
        			var CmsPageItem = new Element('div',{id:'sbs_'+this.id+'_cmspage_index_'+key_cmspage,style:'cursor:pointer;',onclick:'Autocomplete.instances['+this.instanceId+'].select('+key_cmspage+', this)',onmouseover:'$(this).addClassName("selected")',onmouseout:'$(this).removeClassName("selected")'}).addClassName('cmspages suggested-item');
        			CmsPageItem.update('<span class="sbs_search_suggest_item_title">'+cmsPageName+'</span>');    		
    	    		this.rightSideBar.appendChild(CmsPageItem);
    	    		this.itemCount++;
        		}
        	}
        	this.itemCount++;
    	...
    	//Blogs
    	if(this.suggestBlogs.length > 0)
    	{
    		var viewAllBlogPageUrl = this.options.searchResultUrl+'/blog/?q='+this.currentKeyword;
    		var blogDivider = new Element('div').addClassName('suggest_category_items suggest_divider');
    		var blogDividerText = new Element('span').update('Blogs');
    		blogDivider.appendChild(blogDividerText);
    		var viewAllBlogResult = new Element('span').addClassName('view-all-category').update('<a href="'+viewAllBlogPageUrl+'">'+'View all blogs result'+'</a>');
    		blogDivider.appendChild(viewAllBlogResult);
    		this.rightSideBar.appendChild(blogDivider);
    		
    		for(var key_blog in this.suggestBlogs)
        	{
        		if(!isNaN(key_blog)){
        			var blog = this.suggestBlogs[key_blog];
        			
        			var blogName = blog.name_varchar;
        			
        			var blogItem = new Element('div',{id:'sbs_'+this.id+'_blog_index_'+key_blog,style:'cursor:pointer;',onclick:'Autocomplete.instances['+this.instanceId+'].select('+key_blog+', this)',onmouseover:'$(this).addClassName("selected")',onmouseout:'$(this).removeClassName("selected")'}).addClassName('blogs suggested-item');
        			blogItem.update('<span class="sbs_search_suggest_item_title">'+blogName+'</span>');    		
    	    		this.rightSideBar.appendChild(blogItem);
    	    		this.itemCount++;
        		}
        	}
        	this.itemCount++;
  ...
  processResponse: function(response) {
	  ...
	  if(response && response.responseHeader.params.q){	  
		  ...
		  //Collect cms pages
		  this.suggestCmsPages = [];
		  this.suggestCmsPages = this.manager.response.cmspages;
		  
		//Collect cms pages
		  this.suggestBlogs = [];
		  this.suggestBlogs = this.manager.response.blogs;
	  }
	  
	  this.suggest();
  },
  redirectToCmsPage: function (url)
  {
	  window.location = url;
  },
  ...
  select: function(i, obj)
  {
	...
	else if ($(itemId).hasClassName('cmspages')){
		var cmsPage = this.suggestCmsPages[i];
		this.redirectToCmsPage(cmsPage.url_path_varchar);
	}
	else if ($(itemId).hasClassName('blogs')){
		var blog = this.suggestBlogs[i];
		this.redirectToUrl(blog.url_path_varchar);
	}
	else
	{
		if($(itemId) != undefined) {
  ...
  enterSelect: function( selectedIndex ) {
	  ...
	  this.select(this.selectedIndex, selectedNode);
	  return true;
  },
